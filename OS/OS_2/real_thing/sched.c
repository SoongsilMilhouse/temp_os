...
simulate (int sched)
{
	...
	for (p = 0; p < PROCESS_MAX; p++)	//schedule->0 으로, queue->NULL로 초기화
	{
		int slot;
	...
	for (cpu_time = 0; p_done < process_total; cpu_time++)
	{
		...
		for (; p < process_total; p++)
		{
			Process *pp;
			pp = &processes[p];
			if (pp->arrive_time != cpu_time)	//cpu_time이 처음엔 0이니 arrive_time이 0이 아니면 break; 즉, 첫 프로세스는 도착시간이 0이어야한다.
				break;
			...
			case SCHED_PR:
				{
					...
					highest = PRIORITY_MAX + 1;			//처음 비교를 위해 우선 PRORITY_MAX + 1을 넣는다.
					for(i = 0; i < queue_len; i++) 
						if(queue[i]->priority < highest)	//i번째 큐에 있는 일의 우선순위가 현재 일의 우선순위보다 높다면(숫자로는 작다면) TRUE
						{
							process = queue[i];
							highest = process->priority;	//현재 프로세스에 i번째 일을 넣고 shortest에 i번째 일의  우선순위 값을 넣는다.
						}									//그럼으로써 또 다른 프로세스가 들어왔을 때 비교를 통해 shortest의 값을 수정한다.
				}											//즉, 우선순위가 높은 일이 들어올 때 shortest 값을 변경한다.
				break;									
			default:
